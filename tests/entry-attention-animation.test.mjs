...
test("collection and chapter game entries use a subtle fill pulse without changing layout", () => {
  assert.match(page, /className=\{`game-shortcut-button \$\{unlockedCats\.includes\(chapter\.id === "soybean" \? "taro" : "sauce"\) \? "is-completed" : ""\}`\}/);
  assert.match(page, /"去种芋" : "去做酱"/);
  assert.match(page, /className="map-pouch-button" type="button" aria-label="打开图鉴"/);
  assert.match(css, /\.game-shortcut-button:not\(\.is-completed\),\.map-pouch-button\{animation:entryColorPulse 2\.8s ease-in-out infinite\}/);
