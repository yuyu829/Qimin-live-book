...
test("cards recede along the visual z axis", () => {
  ...
  assert.match(deckCardTransform(1), /translate3d\(0, -50px, -24px\) scale\(0\.95\) rotate\(-1\.2deg\)/);
  assert.match(deckCardTransform(2), /translate3d\(0, -100px, -48px\) scale\(0\.9\) rotate\(1\.2deg\)/);
  assert.match(deckCardTransform(4), /translate3d\(0, -200px, -96px\) scale\(0\.8\) rotate\(1\.2deg\)/);
  assert.equal(DECK_STACK_RISE, 200);
});
