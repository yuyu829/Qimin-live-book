...
test("collection cats show a centered locked badge until their games are completed", () => {
  ...
  assert.match(page, /const unlocked = \(id === "taro" \|\| id === "sauce"\) && unlockedCats\.includes\(id\)/);
  ...
  assert.match(css, /\.pouch-lock-badge\{position:absolute;left:50%;top:50%;[^}]*border-radius:50%[^}]*background:rgba\(104,104,104,\.82\)/);
});
