...
function WorldMap({ onOpen }: { onOpen: (id: Chapter["id"]) => void }) {
  ...
  return (
    <main className="map-page">
      ...
      {volume && <div className="map-volume-backdrop" onClick={() => setVolume(undefined)}><section className="map-volume-popover" onClick={(event) => event.stopPropagation()}><button className="map-volume-close" onClick={() => setVolume(undefined)} aria-label="关闭卷目录"><X /></button><p className="overline">齐民要术 · 章节目录</p><h2>{mapVolumes[volume].title}</h2><div className="map-volume-list">{mapVolumes[volume].items.map((item, index) => { const activeIndex = volume === "soybean" ? 11 : 2; return <button key={item} disabled={index !== activeIndex} onClick={() => onOpen(volume)}>{item}{index === activeIndex && <ChevronRight size={15} />}</button>; })}</div></section></div>}
    </main>
  );
}
