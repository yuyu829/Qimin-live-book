...
function scienceParagraphs(text: string) {
  const suppliedParagraphs = text.trim().split(/\n\s*\n|\n/).map((part) => part.trim()).filter(Boolean);
  if (suppliedParagraphs.length > 1 || text.length <= 140) return suppliedParagraphs;
  const sentences = text.match(/[^。！？；]+[。！？；]?/g)?.map((sentence) => sentence.trim()).filter(Boolean) ?? [text];
  const paragraphs: string[] = [];
  let paragraph = "";
  sentences.forEach((sentence) => {
    if (paragraph && paragraph.length + sentence.length > 110) {
